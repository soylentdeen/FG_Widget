; NAME:
;     C3D_PIPE - Version .6.1
;
; PURPOSE:
;     Automatic Data Reduction Pipeline for C3D
;
; CALLING SEQUENCE:
;     C3D_PIPE, INMANIFEST, OUTMANIFEST
;
; INPUTS:
...
pro c3d_pipe, inmanifest, outmanifest

;** error check
s=size(inmanifest)
if (s[0] ne 0) and (s[1] ne 7) then $
  message, 'c3d_pipe - must provide input manifest file name'
s=size(outmanifest)
if (s[0] ne 0) and (s[1] ne 7) then $
  message, 'c3d_pipe - must provide output manifest file name'
;** get file names
; open file
openr, fileunit, inmanifest, /get_lun, error=err
if err ne 0 then $
  message, 'c3d_pipe - error opening input manifest file - error msg=' $
    + !error_state.msg
; get number
n=0
readf, fileunit, n
...
for i=0,n-1 do if strlen(filelist[i]) eq 0 then $
  message, 'c3d_pipe - invalid entries in input manifest file - aborting'
;** open and run pipe
; open
pipe=drip_new(filelist)
; check for valid mode
if strtrim(pipe->getdata(/mode),2) ne 'C3D' then $
  message, 'c3d_pipe - wrong instrument mode (should be C3D)'
; run data reduction
pipe->run,filelist
;** save data
pipe->save
...
if err ne 0 then $
  message, 'c3d_pipe - error opening output manifest file - error msg=' $
    + !error_state.msg
printf, fileunit, '1'
