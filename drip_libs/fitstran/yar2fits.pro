...
; increment order information to avoid conflicts betwe en modules
...
if (max(ll_idx) gt -1 and max(sl_idx) gt -1) then a[3,ll_idx] = a[3,ll_idx]+3  

; increment LH only if SH data present, increment by 10

sh_idx=where(modu eq 4)
lh_idx=where(modu eq 5)
if (max(lh_idx) gt -1 and max(sh_idx) gt -1) then a[3,lh_idx] = a[3,lh_idx]+10  
