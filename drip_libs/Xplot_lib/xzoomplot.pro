...
;
; CALLING SEQUENCE:
;     xzoomplot,x,y,YTITLE=ytitle,XTITLE=xtitle,TITLE=title,CANCEL=cancel       ; 
; INPUTS:
...
if not xregistered('xzoomplot_help') then begin

    getfonts,buttonfont,textfont

    help_base = widget_base(GROUP_LEADER=state.w.xzoomplot_base, $
                            /COLUMN, $
                            TITLE='Xzoomplot Help')
...
if keyword_set(PS) then begin
    ...
    plot,*state.d.owave,*state.d.oflux,/XSTY,/YSTY,$
      ...
      /NODATA,CHARTHICK=state.p.thick,THICK=state.p.thick,$
...
plot,*state.d.owave,*state.d.oflux,/XSTY,/YSTY,YRANGE=state.p.plotyrange,$
  ...
  TITLE=state.p.title,CHARSIZE=state.p.charsize,XTHICK=state.p.thick,$
  YTHICK=state.p.thick
    ...
    'Keyboard': begin
        ...
        case strtrim(event.ch,2) of 
            ...
            'w': begin
                ...
                state.p.plotyrange = [min(*state.d.oflux,MAX=max,/NAN),max]
                xzoomplot_plotspec
                xzoomplot_setminmax
...
;******************************************************************************
;
pro xzoomplot,x,y,YTITLE=ytitle,XTITLE=xtitle,TITLE=title,CANCEL=cancel
...
if n_params() eq 2 then begin
    ...
    if cancel then return
    cancel = cpar('xzoomplot',y,1,'Y',[2,3,4,5],1)
    if cancel then return
