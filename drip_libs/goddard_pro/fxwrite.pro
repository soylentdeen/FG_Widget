...
;
	IF N_PARAMS() EQ 2 THEN BEGIN
		IF (FXPAR(HEADER,'NAXIS') NE 0) THEN BEGIN
			MESSAGE = 'NAXIS should be zero for no primary ' + $
				'data array'
			IF N_ELEMENTS(ERRMSG) NE 0 THEN BEGIN
				ERRMSG = MESSAGE
				RETURN
		...
		END ELSE IF (NOT FXPAR(HEADER,'EXTEND')) THEN BEGIN
			MESSAGE = 'EXTEND should be true for no primary ' + $
				'data array'
			IF N_ELEMENTS(ERRMSG) NE 0 THEN BEGIN
				ERRMSG = MESSAGE
				RETURN
	...
	END ELSE BEGIN
		...
		IF !ERR NE 0 THEN BEGIN
			MESSAGE = 'Error occurred checking FITS header'
			IF N_ELEMENTS(ERRMSG) NE 0 THEN BEGIN
				ERRMSG = MESSAGE
				RETURN
			END ELSE MESSAGE, MESSAGE
...
;
       	GET_LUN, UNIT      
       	OPENW, UNIT, FILENAME, 2880, /BLOCK
;
