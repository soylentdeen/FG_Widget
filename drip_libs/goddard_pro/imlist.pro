...
REDO:
 case 1 of                                    ;Get proper print format
   ...
   type EQ 2:  fmt = '(i4,1x,' + cdx + 'i' + sfmt + ')'        ;Integer

   (type EQ 4) or (type EQ 3) or (type EQ 5):  begin      ;Long, Real or Double

      temp = image[ xmin:xmax,ymin:ymax ]
      minval = min( temp, MAX = maxval)
      if type EQ 3 then if (maxval LT 99999) and (minval GT -9999) then begin
		type = 2
		goto, REDO
      endif
