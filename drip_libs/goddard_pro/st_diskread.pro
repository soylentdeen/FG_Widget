		...
		if strpos(strlowcase(name),'jit') eq -1 then begin; text trailer
		  ...
		  ;
                  ...
                  for j= 0, tfields-1 do begin
                  	val=ftget(htab,tab,j+1)     ; extract column
                        ftinfo,htab,j+1,d1,d2,d3,d4,d5,d6,d7,d8,fname
                        tab_put,strtrim(fname),val,tcb,otab
                  end
...
;
	...
	if !err lt 0 then bscale=1.
	bzero = sxpar(h,'bzero')
	if !err lt 0 then bzero=0.
...
;
	opsize = sxpar(h,'opsize')
	if !err lt 0 then opsize = 0
	opsize = opsize/8
;
; set up data array
;
