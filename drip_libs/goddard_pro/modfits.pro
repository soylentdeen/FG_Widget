pro MODFITS, filename, data, header, EXTEN_NO = exten_no
;+
; NAME:
;      MODFITS
; PURPOSE:
...
; Check for filename input
   ...
   if (nheader GT 1) and (ndata GT 1) then begin
        check_fits, data, header, /FITS
        if !ERR EQ -1 then goto, BAD_EXIT
   endif
...
; Open file and read header information
   ...
   if exten_no EQ 0 then begin 
         if nheader GT 0 then $
             if strmid( header[0], 0, 8)  NE 'SIMPLE  ' then message, $
                'ERROR - Input header does not contain required SIMPLE keyword'
         ...
         if unit EQ -1 then goto, BAD_EXIT
   endelse
   ...
   if nheader GT 1 then begin
      ...
      if (nheader/36) NE (Noldheader/36) then begin
        message,'ERROR - FITS header not compatible with existing file ',/con
   ...
   if ndata GT 1 then begin
        ...
        if (newbytes/2880) NE (nbytes/2880) then begin
        message,'ERROR - FITS data not compatible with existing file ',/con
        message,'Input FITS data contains '+ strtrim(newbytes,2) + ' bytes',/inf
        message,'Current disk FITS header contains ' + strtrim(nbytes,2) + $
                ' bytes',/inf
        goto, BAD_EXIT
        endif
        if nheader EQ 0 then begin
                check_fits,data,oldheader,/FITS
                if !ERR EQ 1 then goto, BAD_EXIT
        endif
