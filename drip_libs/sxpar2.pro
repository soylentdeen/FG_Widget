...
;  string.
   ...
   if strpos( nam, '*' ) EQ namelength1 then begin
            ...
            num_length = 9 - name_length        ;Max length of number portion
            if num_length LE 0 then  $
                  message, 'Keyword length must be 9 characters or less'

;  Otherwise, extend NAME with blanks to eight characters.

    endif else begin
                while strlen(nam) LT 9 do nam = nam + ' ' ;Make 9 chars long
                vector = 0
    endelse
...
;  value of the number field in NUMBER.
        ...
        if (not vector) and (start GE 0) then begin
            ...
            keyword = strmid(hdr[mn:mx], 0, 9)
        endif else begin
            restart:
            start   = -1l
            keyword = strmid( hdr, 0, 9)
        endelse
...
; Process string parameter
 ...
 if matches GT 0 then begin
  line = hdr[nfound]
  svalue = strtrim( strmid(line,10,71),2)
  if histnam then $
        value = strtrim(strmid(line,9,71),2) else for i = 0,matches-1 do begin
                ...
                On_IOerror, GOT_VALUE
                  ...
                  or (strpos(value,'D') GE 0) then begin  ;Floating or double?
                      if ( strpos(value,'D') GT 0 ) or $  ;Double?
                         ( strlen(value) GE 9 ) then value = double(value) $
