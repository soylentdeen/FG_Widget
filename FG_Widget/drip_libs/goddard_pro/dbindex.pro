...
;	by DBOPEN 
;
;	(2) Only 15 items can be indexed at one time.   If the database has
;	more than 15 items, then two separate calls to DBINDEX are needed.
; PROCEDURES CALLED:
...
; Extract index items from data base
 ...
 if Nindex LE 0 then begin
	...
	return
 endif else if Nindex GT 15 then begin
	message,'ERROR - Only 15 items can be indexed at one time',/INF
	return
 endif
...
; extract index items...maximum of 15 indexed fields.
