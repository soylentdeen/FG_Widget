...
;
; PURPOSE:
;     Data Reduction Pipeline for two position chop two position nod mode where
;   one chop position is on the slit, and the other is off.
;
; CALLING SEQUENCE:
;     Obj=Obj_new('NOS', FILELIST)
...
;     Modified:  Casey Deen, UT/IC, October 2011
;                Used nas__define.pro to creat new grism mode nos__define.pro
...
; create README
self.readme=['pipeline: Nod Off Slit', $ ;info lines
  'file: ' + self.filename, $
  'final image: undistorted', $
  'order: CLEAN, NONLIN, FLAT, STACK, UNDISTORT, MERGE, COADD', $
  'notes: badmap from CLEAN, masterflat from FLAT']

print,'NOS FINISHED' ;info
end

;******************************************************************************
;     NOS__DEFINE - Define the NOS class structure.
;******************************************************************************

pro nos__define  ;structure definition
