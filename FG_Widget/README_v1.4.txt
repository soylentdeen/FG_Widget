...
FG_WIDGET v1.4 README

Last Update: May 21st, 2010
...
1) Download the following:
    ...
    source tarball file: FG_Widget.tgz
     
    tar -zxf FG_Widget.tgz (produces a directory called FG_Widget)
...
IDL> @/yourpath/FG_Widget/@fg_start.
...
(A or B). Size the colored extraction box using left-click and drag on the right or bottom edge of the box to enclose the part of the image you want to extract to a 1-D spectrum. Note that you can drag the right side of the box up or down to produce a parallelogram to enclose a slanted order.

7) Click the 'Extract' button in the lower left of the GUI and choose
either 'Extract Selected Region' or one of the preset grism modes 
(G1, G1xG2, G3, G3xG4, G5, or G6).

8) Your spectrum should appear in the large plot window (for 'Extract Selected Region) or in a new window if you extracted a preset grism mode. These windows have all of the functionality of 'xzoomplot.pro' used in SpeXTool and then some.
...
10) If you use extract a cross-dispersed grism spectrum, a window will pop up with all orders plotted at once.  For single order modes (e.g. 'Extract --> G5') fg_widget extracts a roughly wavelength calibrated spectrum.
...
-- Optimal extraction of spectra from the ROI (Fully coded, integration in
process)
