...
;      data, cleaned, badflags, flatted, merged, undistorted - pipe steps
;      coadded, lastcoadd                                    - final products
;      badmap, flats, cleanedflats, masterflat, darksum}     - calibration
;
; OUTPUTS:
...
;
; PROCEDURE:
;     Upon initialisation get mode. For each file check if the mode is
;     correct and reduce the file. All files are coaddecd. Run returns
;     the data structure.
...
pro drip::setdata, mode=mo, n=n, readme=rm, $
        ...
        badfile=bf, flatfile=ff, data=da, $
        cleaned=cl, badflags=bl, flatted=fl, stacked=st, undistorted=ud, $
...
if keyword_set(ds) then *self.darks=ds
...
structure={mode:self.mode, n:self.n, readme:self.readme, header:*self.header, $
           ...
           badfile:self.badfile, flatfile:self.flatfile, $
           data:*self.data, cleaned:*self.cleaned, badflags:*self.badflags, $
           flatted:*self.flatted, stacked:*self.stacked, $
...
pro drip::load, filename, masterflat=mf, cleaned=cl, flatted=fl, stacked=st, $
...
pro drip::save, masterflat=mf, cleaned=cl, flatted=fl, stacked=st, $
                undistort=ud, merged=md, coadded=co, coadded_rot= cor, $
                filename=filename
...
;******************************************************************************
;     GETCAL - get support files (flat, dark and bad)
;******************************************************************************
...
if self.badfile eq 'x' then begin
    ...
    *self.badmap=readfits(self.badfile,badhead)
...
if self.darkfile eq 'x' then begin
    ...
    *self.darks=readfits(self.darkfile,darkhead)
...
if self.flatfile eq 'x' then begin
    ...
    *self.flats=readfits(self.flatfile,flathead)
    if size(*self.flats,/N_dimensions) gt 0 then begin
        ...
        ; no valid data loaded - set flat to 1.0
...
if s[0] gt 2 then flatsum=total(*self.cleanedflats,3) else flatsum=*self.cleanedflats

; make masterflat
filter=drip_getpar(*self.basehead,'FILT2_S')
if (filter eq 'grism2') then begin
    ;get the order mask
    fname=drip_getpar(*self.basehead,'ORDERMASK')
    ordermask=readfits(self.pathload+fname)
endif else begin
   ordermask=dblarr(256,256)+1
endelse
master=(flatsum)*ordermask ; flatsum-darksum
master=master/median(master[where(ordermask eq 1)]) ; normalize
if (filter eq 'G1xG2') then begin
    master[where(ordermask eq 0)]=1
endif
*self.masterflat=(master);>0.00001) ; to have no pixels equal to zero
...
sxaddpar,*self.basehead,'PIPEVERS','Forcast_FGWIDGET_1.3'
end
...
if self.n gt 0 then begin
    ...
    *self.coadded=*self.coadded+*self.undistorted
    ;*self.coadded_rot=*self.coadded_rot+*self.undistorted
endif else begin
    *self.coadded=*self.undistorted
    ;*self.coadded_rot=*self.undistorted
    *self.lastcoadd=*self.undistorted
    (*self.lastcoadd)[*,*]=0.0
endelse
; create readme
print,'DRIP FINISHED'
...
for filei=0,lists[1]-1 do begin
    ...
    ; load file
    data=readfits(self.filename,*self.header)
